...
To set the font, we'll add another argument to `bs_theme`. We'll set it to `base_font = font_google("Lato", wght = "200..900", local = FALSE)`.
...
This code isn't too important for this tutorial, but it ensures that we import all the required weights (semi-bold, bold, etc.) for the Lato font.
